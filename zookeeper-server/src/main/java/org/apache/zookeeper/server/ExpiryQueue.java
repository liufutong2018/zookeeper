...
public class ExpiryQueue<E> {
    ...
    public ExpiryQueue(int expirationInterval) {
        ...
        // nextExpirationTime用于记录下次要进行过期会话清理的时间点
     ...
     */ // 当Client与Server有交互时（连接请求、读写操作、心跳），该方法就会调用
        // 当zk server启动时会将磁盘的session恢复到内存，也会调用该方法
        // 该方法在做的是会话换桶操作
    public Long update(E elem, int timeout) {

        // elemMap集合的key为session，value为该session的过期时间，即该session当前所在的会话桶id
        ...
        // 若之前所在会话桶id与本次交互计算的会话桶id相同，则无需换桶，即什么也不用做
        ...
        // expiryMap是会话桶集合，其value为会话桶(Set集合)，key为该会话id(其表示的时间范围的最大边界值)
        ...
        // 返回当前交互引发的会话所在的会话桶id，即当前会话的真正过期时间点
